class AwardTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @award_type.save
        format.html { redirect_to(@award_type, :notice => 'AwardType was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @award_type.update_attributes(params[:award_type])
        format.html { redirect_to(@award_type, :notice => 'AwardType was successfully updated.') }
        format.xml  { head :ok }
      else
